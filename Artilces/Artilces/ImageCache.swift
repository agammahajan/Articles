...
extension UIImageView {
	func setImageWithUrl(urlString: NSString, placeholderImage: UIImage?) {
		self.image = placeholderImage
		ImageCache.sharedInstance.fetchImageWith(urlString: urlString, forInstance: self) { (image) in
			self.image = image
		}
	}
}
